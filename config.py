...
MIN_TRADE_INTERVAL_TRENDING = 15  # 추세장에서는 쿨다운 단축

# Dynamic stop-loss (ATR-based): stop_loss = max(STOP_LOSS_PCT, ATR * multiplier / price)
DYNAMIC_STOP_LOSS_ENABLED = True
DYNAMIC_STOP_LOSS_ATR_MULT = 2.0  # ATR x 2 = dynamic stop distance

# Tiered take-profit settings (more granular for better profit capture)
TIERED_TAKE_PROFIT = [
    {"threshold": 0.03, "sell_pct": 10, "condition": "momentum_weakening"},
    {"threshold": 0.05, "sell_pct": 15, "condition": "momentum_weakening"},
    {"threshold": 0.08, "sell_pct": 20, "condition": "always"},
    {"threshold": 0.12, "sell_pct": 30, "condition": "always"},
    {"threshold": 0.18, "sell_pct": 40, "condition": "always"},
    {"threshold": 0.25, "sell_pct": 50, "condition": "always"},
]

# RSI-based accumulation: buy more aggressively when RSI is deeply oversold
RSI_OVERSOLD_ACCUMULATION_ENABLED = True
RSI_DEEP_OVERSOLD = 25  # RSI < 25 → aggressive buy boost
RSI_OVERSOLD_BOOST = 1.5  # 50% larger position on deep oversold
